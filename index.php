<?php
	/*
		...
		Yubo Yang
		Augest 1, 2022
		Description: This page is the home page, it listed all diaries, and there are different categories on the header.
	*/
	...
	//$query ="SELECT d.diary_id, d.title d_title, d.content d_content, d.image, d.category_id, d.create_date d_create_date, d.update_date, ca.name FROM diaries d, categories ca WHERE d.category_id = ca.category_id  ORDER BY {SELECT item FROM sortby} DESC";
...
<body>
	<div id="wrapper">
		...
		<ul id="menu">
			...
			<li><a href="category_filter.php?id=<?= $row['category_id'] ?>"><?= $row['name'] ?></a></li>
			<?php endwhile ?>
		</ul>
		<div id="all_blogs">
